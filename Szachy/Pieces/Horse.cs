...
namespace Szachy.Pieces
{
    class Horse : Piece
    {
        public Horse(byte id, Bitmap image, byte[] board) : base(id, image, board)
        {
        } //moze pozostac pusty
        ...
        public override bool move(byte moveFrom, byte moveTo)
        {
            ...
            if ((Math.Abs(moveFromX - moveToX) == 1 && Math.Abs(moveFromY - moveToY) == 2) ||
                (Math.Abs(moveFromX - moveToX) == 2 && Math.Abs(moveFromY - moveToY) == 1)) return true;
